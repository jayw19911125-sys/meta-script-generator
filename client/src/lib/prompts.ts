...
| 層級 | Hook 目標 | 允許的公式 | 禁止 |
|---|---|---|---|
| TOFU | 喚醒問題意識 | 痛點、提問、故事 | 不能提產品名、不能賣 |
| MOFU | 製造差異化好奇 | 反常識、結果先行、提問 | 不能太軟 |
| BOFU | 臨門一腳衝動 | 結果先行、視覺、社會證明 | 不能再教育 |
...
export function buildGptPrompt(data: PromptInput): string {
...
export function buildClaudePrompt(data: PromptInput, gptHooks: string): string {
  return `## 任務
...
export function buildGptIntegratePrompt(data: PromptInput, hooks: string): string {
