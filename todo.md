...
- [ ] 缺陷3：知識庫命中率指標 — 腳本生成結果顯示知識庫是否有效被引用
- [ ] 缺陷4：雙引擎說明 — 在 UI 中清楚說明 GPT 發散 + Claude 整合的分工
- [ ] 缺陷9：腳本品質評分 — 預估秒數、Hook 強度（1-5）、CTA 明確度（1-5）
- [ ] 缺陷5：矩陣組合排序機制 — 加入優先分數排序，避免組合爆炸
- [ ] 缺陷6：腳本版本控管 — 歷史紀錄保存 AI 原始版本，支援 diff 比對
- [ ] 缺陷7：匯出直接推送 Notion — 腳本生成後可一鍵推送至 Notion 指定頁面
- [ ] 缺陷8：歷史紀錄搜尋 — 支援關鍵字、產品名稱、日期範圍篩選
- [ ] 缺陷15：補強核心業務邏輯測試覆蓋率（腳本生成、矩陣、知識庫同步）
